...
source=Path(sys.argv[1]) if len(sys.argv)>1 else root/'exports/aurorian_cow/aurorian_cow_panic_preview.bbmodel'
model=json.loads(source.read_text(encoding='utf-8-sig'))
names={'idle_breathe','idle_chew_look','idle_sniff','idle_tail_swish','walk_relaxed','startle','run_scared'}
def f(v):
    s=format(float(v),'.9g')
...
lines=['package cn.teampancake.theaurorian2.client.animation;', '',
 ...
 'public final class AurorianCowAnimations {','    private AurorianCowAnimations() {}']
found=set()
...
assert found==names
lines.append('}')
dest=root/'src/main/java/cn/teampancake/theaurorian2/client/animation/AurorianCowAnimations.java'
dest.parent.mkdir(parents=True,exist_ok=True)
dest.write_text('\n'.join(lines)+'\n',encoding='utf-8')
print('Exported all 7 approved animations without changing keyframe times or values beyond axis conversion.')
