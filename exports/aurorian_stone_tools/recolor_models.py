...
NAMES={'pickaxe':'极光石镐','axe':'极光石斧','hoe':'极光石锄','shovel':'极光石铲'}
...
for key,name in NAMES.items():
 ...
 for y in range(64):
  for x in range(64):
   ...
   if warm:rgb=ramp((lum-40)/115,[(47,43,77),(100,110,141),(157,186,194)])
   elif (x,y) in handle:rgb=ramp((lum-65)/190,[(33,44,109),(35,116,180),(78,194,222)])
   else:rgb=ramp((lum-45)/210,[(25,26,36),(56,68,86),(93,126,135)])
   new.putpixel((x,y),(*rgb,a))
 assert np.array_equal(np.array(old)[:,:,3],np.array(new)[:,:,3])
 filename=f'aurorian_stone_{key}_recolor.png';new.save(R/filename)
 # Preview-only game-format JSON: preserve every geometry/UV/display/group field.
 exported=copy.deepcopy(src)
 exported['textures']={k:('theaurorian2:item/aurorian_stone_'+key+'_recolor' if not v.startswith('#') else v)for k,v in src['textures'].items()}
 (R/f'aurorian_stone_{key}.json').write_text(json.dumps(exported,ensure_ascii=False,separators=(',',':')),encoding='utf-8')
 ...
 (R/f'aurorian_stone_{key}.bbmodel').write_text(json.dumps(bb,ensure_ascii=False,separators=(',',':')),encoding='utf-8')
...
d.text((60,32),'极光石工具 · 游戏模型',font=font(44),fill='#ecf4ff')
d.text((62,102),'直接使用 Adventurer Tools 模型 · 保留造型和 UV · 深蓝灰石质工具头 · 木柄与蓝色装饰',font=font(22),fill='#9fb6c9')
