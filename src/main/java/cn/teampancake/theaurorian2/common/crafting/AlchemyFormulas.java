...
import cn.teampancake.theaurorian2.common.registry.AlchemyFormulaData;
...
public final class AlchemyFormulas {
    ...
    private static boolean isBitSet(int data, int bitPosition) {
        return ((data & 1 << Math.floorMod(bitPosition, 15)) != 0);
    }
    ...
    private static int parseEffectFormula(String formula, int start, int end, int data) {
        ...
        if (orIndex >= 0 && orIndex < end) {
            int leftResult = parseEffectFormula(formula, start, orIndex, data);
            if (leftResult > 0) {
                return leftResult;
            }
        ...
        if (andIndex >= 0 && andIndex < end) {
            int leftResult = parseEffectFormula(formula, start, andIndex, data);
            if (leftResult <= 0) {
                return 0;
            }
    ...
    public static List<MobEffectInstance> getPotionEffects(int damage) {
        ...
        for (Holder.Reference<MobEffect> reference : BuiltInRegistries.MOB_EFFECT.listElements().toList()) {
            AlchemyFormulaData.Formula usableEffectData = reference.getData(AlchemyFormulaData.EFFECTS);
            if (usableEffectData != null) {
                ...
                if (duration > 0) {
                    int amplifier = 0;
                    AlchemyFormulaData.Formula amplifierEffectData = reference.getData(AlchemyFormulaData.AMPLIFIERS);
    ...
    public static int calculateNetherWartPhase1(int liquidData) {
        ...
        byte highestBit = 14;
        while (highestBit >= 0 && (liquidData & 1 << highestBit) == 0) {
            highestBit--;
        }
    ...
    public static int calculateNetherWartPhase2(int liquidData) {
        ...
        BitSet visited = new BitSet(32768);
        while (previousResult != result && !visited.get(liquidData)) {
            visited.set(liquidData);
